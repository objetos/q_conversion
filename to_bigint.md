...
Each `1` bit marks a filled cell and `0` an empty one. Cells are encoded in **row-major** order, and by default, use **big-endian** layout (top-left cell is the most significant bit). Round-trips with [createQuadrille(width, bitboard, value)]({{< relref "create_quadrille_width_bigint_value" >}}) — the backbone of the `maze` → `toBigInt` → `const` level pipeline (see [maze]({{< relref "maze" >}})).
